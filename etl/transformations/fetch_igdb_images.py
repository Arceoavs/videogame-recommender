...
with open(csv_name, 'a', newline='', encoding='utf-8') as csvfile:
    ...
    if(maxid)==-1:
        spamwriter.writerow(["igdb_id", "image_link"])

    igdb_games = pd.read_sql_query(
        '''
        SELECT id,
               NAME
        FROM   igdb.stage_games
        WHERE  id IN (SELECT igdb_id
                      FROM   lookup.games
    ...
    for index, row in igdb_games.iterrows():
