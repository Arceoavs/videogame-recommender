...
csv_name='../datasets/metacritic/images.csv'

if path.exists(csv_name):
    data = pd.read_csv(csv_name, delimiter=';')
    try:
        maxid=max(data['metacritic_id'])
...
with open(csv_name, 'a', newline='', encoding='utf-8') as csvfile:
    ...
    if(maxid)==-1:
        spamwriter.writerow(["metacritic_id", "image_link"])

    metacritic_games = pd.read_sql_query(
        '''
        SELECT id,
               title
        FROM   metacritic.stage_games
        WHERE  id IN (SELECT metacritic_id
                      FROM   lookup.games
    ...
    for index, row in metacritic_games.iterrows():
        response = requests.get(
            BASE_URL,
            params={'search':row['title']}
        )
        id = row['id']
        try:
