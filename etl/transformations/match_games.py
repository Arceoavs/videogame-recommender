...
year_filter = True
platfrom_filter = False
last_number_filter = True
most_similar_titles = True
...
def apply_metadata_filter(df, left_prefix, right_prefix):
    # Matchings should have:
    # A similarity of 1 (same game title)
    # Or provide metadata, i.e. year or platform information
    new_df = df[(df['_sim_score'] == 1.0) | (
        df[f'{left_prefix}_year'].notnull() & df[f'{right_prefix}_year'].notnull()) | (
        df[f'{left_prefix}_platforms'].notnull() & df[f'{right_prefix}_platforms'].notnull())]
    print(f'[Metadata Filter] Before     {len(df)}')
    print(f'[Metadata Filter] Filtered   {len(df) - len(new_df)}')
    print(f'[Metadata Filter] After      {len(new_df)}')
    return new_df


def apply_year_filter(df, left_prefix, right_prefix):
    new_df = df[
        ((df[f'{left_prefix}_year'].notnull()) & (df[f'{right_prefix}_year'].notnull()) & (abs(df[f'{left_prefix}_year']-df[f'{right_prefix}_year']) <= max_year_diff)) |
...
def apply_platform_filter(df, left_prefix, right_prefix):
    ...
    new_df = df[df.apply(lambda r: (not r[f'{left_prefix}_platforms']) or (not r[f'{right_prefix}_platforms']) or not (
        set(r[f'{left_prefix}_platforms'].split(', ') if r[f'{left_prefix}_platforms'] else []).isdisjoint(set(r[f'{right_prefix}_platforms'].split(', ') if r[f'{right_prefix}_platforms'] else []))),
        axis=1)]
...
def find_most_similar_titles(candidates, left, right):
    new_candidates = candidates
    grouped = candidates.groupby(f'{right}_id').filter(
        lambda x: len(x) > 1).groupby(f'{right}_id')
    for name, group in grouped:
        highest_sim = 0.0
        sim_item = None
        for index, row in group.iterrows():
            current_item = row[f'{right}_id']
            sim = lev.get_sim_score(clean_string(
                row[f'{left}_title']), clean_string(row[f'{right}_title']))
            if sim > highest_sim:
                highest_sim = sim
                if sim_item is not None:
                    new_candidates = new_candidates[(new_candidates[f'{right}_id'] != current_item) & (
                        new_candidates[f'{left}_id'] != sim_item)]
...
def process_candidates(candidates, left, right):
    print(
        f'[Match {left}-{right} games] {len(candidates)} potential matching candidates')

    if metadata_filter:
        candidates = apply_metadata_filter(candidates, left, right)
    if year_filter:
        candidates = apply_year_filter(candidates, left, right)
    if platfrom_filter:
        candidates = apply_platform_filter(candidates, left, right)
    if last_number_filter:
        candidates = apply_last_number_filter(candidates, left, right)

    left_best = candidates[[f'{left}_id', '_sim_score']].sort_values(
        '_sim_score', ascending=False).groupby(candidates[f'{left}_id'], as_index=False).first()
    right_best = candidates[[f'{right}_id', '_sim_score']].sort_values(
        '_sim_score', ascending=False).groupby(candidates[f'{right}_id'], as_index=False).first()

    candidates = pd.merge(pd.merge(candidates, left_best, on=f'{left}_id', suffixes=(
        f'_candidates', f'_{left}')), right_best, on=f'{right}_id')
    print(
        f'[Match {left}-{right} games] {len(candidates)} matches after joining results')
    candidates = candidates[(candidates._sim_score_candidates == candidates[f'_sim_score_{left}']) & (
        candidates._sim_score_candidates == candidates._sim_score)]
    print(
        f'[Match {left}-{right} games] {len(candidates)} matches after choosing most similar matches')

    if find_most_similar_titles:
        candidates = find_most_similar_titles(candidates, left, right)
        print(f'[Match {left}-{right} games] {len(candidates)} matches after finding the best matches by title similarity from the most similar matches')

    candidates.to_sql(f'{left}_{right}_game_candidates', engine,
                      schema='matching', if_exists='replace', index_label='key')
...
with engine.connect() as connection:
    ...
    ig_candidates.to_sql('games_ig', engine, schema='matching',
                         if_exists='replace', index_label='key')
    ...
    im_candidates.to_sql('games_im', engine, schema='matching',
                         if_exists='replace', index_label='key')
    ...
    gm_candidates = process_candidates(
        gm_candidates, 'giantbomb', 'metacritic')
    gm_candidates.to_sql('games_gm', engine, schema='matching',
                         if_exists='replace', index_label='key')
    ...
    matches = matches.append(unmatched_giantbomb, ignore_index=True).append(
        unmatched_metacritic, ignore_index=True)
    matches['id'] = np.NaN  # default value

    generated_id = 1

    # Addign ids for all common IGDB games
    grouped = matches.groupby('igdb_id').filter(
        lambda x: len(x) > 1).groupby('igdb_id')
    for name, group in grouped:
        for index, row in group.iterrows():
    ...
    grouped = matches[matches['id'].isnull()].groupby(
        'giantbomb_id').filter(lambda x: len(x) > 1).groupby('giantbomb_id')
    for name, group in grouped:
        for index, row in group.iterrows():
            matches.loc[index, 'id'] = generated_id
        generated_id += 1
    ...
    grouped = matches[matches['id'].isnull()].groupby(
        'metacritic_id').filter(lambda x: len(x) > 1).groupby('metacritic_id')
    for name, group in grouped:
        for index, row in group.iterrows():
            matches.loc[index, 'id'] = generated_id
        generated_id += 1
